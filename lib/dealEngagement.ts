...
//
// "in" is here deliberately, not a typo for "inc": confirmed live that
// Dealer-name matching rules now live in dealerName.ts so client code can use
// them without importing this module (and node:crypto with it). Re-exported
// here because dealerEmail.ts and this module's own tests import them from
// dealEngagement, and moving those imports would be churn for no gain.
import { normalizeDealerKey } from "./dealerName";
export { normalizeDealerKey, looksLikeTruncatedDealerName } from "./dealerName";
