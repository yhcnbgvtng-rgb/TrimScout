...
export const DealerPortal: React.FC<DealerPortalProps> = ({
  ...
  // Real won deals (from the actual deals/deal_bids tables) — separate
  // from `wonDeal` above, which is still the demo lockedDeal prop for the
  // delivery-method flow. This is what the AI-verified paperwork upload
  // below actually operates on, since it needs a real deal id.
  const [realWonDeals, setRealWonDeals] = useState<DealerWonDeal[]>([]);
  ...
  const refreshWonDeals = () => {
    fetch("/api/dealer-won-deals")
      .then((res) => (res.ok ? res.json() : { wonDeals: [] }))
      .then((json) => setRealWonDeals(json.wonDeals || []))
      .catch(() => {
        // Best-effort — the rest of the portal still works without this.
      });
  };

  const handleUploadContract = async (dealId: string) => {
    if (!contractUploadFile) return;
    setUploadingDealId(dealId);
    setUploadError(null);
    try {
      const body = new FormData();
      body.append("file", contractUploadFile);
      const res = await fetch(`/api/deals/${dealId}/contract`, { method: "POST", body });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 animate-fadeIn">
      ...
      {/* REAL AI-VERIFIED PAPERWORK UPLOAD — operates on the actual deals
          table, independent of the demo lockedDeal/wonDeal flow above. */}
      {activeTab === "locked_deals" &&
        realWonDeals
          .filter((wd) => wd.dealId)
          .map((wd) => {
            const dealId = wd.dealId as string;
            const v: ContractVerificationResult | null = wd.verification;
            return (
              <div key={dealId} className="rounded-2xl border-2 border-blue-500/50 bg-surface p-6 space-y-4 shadow-xl">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
                  <div>
                    <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-blue-400">
                      <ShieldCheck className="h-4 w-4" /> AI-Verified Paperwork
                    </div>
                    <h3 className="text-base font-black text-white mt-0.5">{wd.bid.matchedVehicleTitle}</h3>
                    <p className="text-xs text-ink-muted">
                      Buyer: {wd.buyerName} · Winning bid: {formatCurrency(wd.bid.totalOtdPrice)}
                    </p>
                  </div>
                  <span
                ...
                {uploadError && (
                  <div className="flex items-center gap-2 rounded-xl border border-rose-500/50 bg-rose-950/30 p-3 text-[11px] text-rose-200">
                    <AlertCircle className="h-3.5 w-3.5 shrink-0" /> {uploadError}
                  </div>
                )}

                <div className="flex flex-wrap items-center gap-3">
                  <label className="flex items-center gap-2 rounded-xl border border-border bg-background px-3.5 py-2 text-xs font-semibold text-ink-light hover:border-blue-500/60 cursor-pointer transition-all">
                    <UploadCloud className="h-4 w-4 text-blue-400" />
                    {contractUploadFile ? contractUploadFile.name : "Choose signed contract PDF…"}
                    <input
                      type="file"
                  ...
                  <button
                    type="button"
                    onClick={() => handleUploadContract(dealId)}
                    disabled={!contractUploadFile || uploadingDealId === dealId}
                    className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-xs font-black text-white hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                    {uploadingDealId === dealId ? (
                      <>
                      ...
                      <>
                        ...
                        {wd.paperworkStatus === "uploaded" ? "Re-upload & re-verify" : "Upload & verify with AI"}
                      </>
                    )}
                  </button>
                </div>
                <p className="text-[10px] text-ink-faint">
                  AI reads the contract's stated total price and dealer name and checks them against your winning
