...
- `merge_<brand>.py` — locator + staff page → upload rows (email precedence: staff page → locator email that matches the GM → name only)
- `scripts/probes/push-<brand>-roster.mts` — bulk upsert into the live directory (existing contacts are kept when ours has no email)
