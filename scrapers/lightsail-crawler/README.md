...
**One canonical date per driver run, inherited by every brand subprocess — not recomputed per brand.** `run-daily-crawl.mjs` computes its own `date` once (via `easternDateStamp()`) at the very start of `main()` and threads it through every state/brand it runs that invocation. `standalone.js` (the actual per-brand crawl, spawned once per brand as its own subprocess) used to compute its own `todayDate` independently via `easternDateStamp()` at whatever wall-clock moment that specific subprocess happened to start — fine for a short run, but a real bug on a run that crosses Eastern midnight mid-flight (confirmed live 2026-09-16: a ~9pm ET start with FL and TX, the two biggest states, still crawling past midnight ET — 11 of 61 brand-runs computed the next day's date and filed into a freshly-created `daily_changes_<next-date>.json` instead of the run's actual date, splitting one business day's ledger across two files). Fixed by having the driver pass its canonical `date` down as the `CRAWLER_RUN_DATE` env var (`YYYY-MM-DD`, Eastern — see `buildBrandCrawlEnv()` in `scripts/run-daily-crawl.mjs`) for every brand subprocess it spawns; `standalone.js`'s `todayDate` is now `resolveRunDate()` (`src/date_utils.js`), which uses `CRAWLER_RUN_DATE` when present and falls back to computing the Eastern date fresh only when it's unset — so a manual/ad hoc `node src/standalone.js` run (not launched by the driver — the standard way every prior state rollout has sample-tested new state data) is unaffected. This only ever changes which day a brand's data is *filed under*; it touches every one of `todayDate`'s existing uses (DOM snapshot capture/pruning, `firstSeen`/`lastSeen`/`soldDate` in `inventory_merge.js`, the `daily_changes_<date>.json` filename) identically, since they all read the same `todayDate` constant. `scripts/dealer-bot-report.mjs` had the identical independently-computed-date pattern (also spawned by the driver, its own header comment already flagged the unenforced "the two must always agree" assumption) and got the same fix.

### Daily driver: NJ + NY + FL + GA + TX + SC + VA, every brand, one job
...
Adding VA (2026-09-16) as a seventh state needed no pipeline changes beyond the same mechanical set: `src/states.js`, `va_policy.js`, `write-va-dealer-files.mjs`, and the two loader-registry entries. The one real cross-state collision the state-scoped dedup key caught this round: `acura-dealers.json` carries a "Crown Acura" in both FL (`crownacura.com`) and VA/Richmond (`crownacurarichmond.com`) — genuinely distinct rooftops, both confirmed surviving in `test/nj_ops.test.js`'s VA block. VA is larger than SC (~42,000 sq mi vs ~32,000) with a genuinely distinct, dense Northern VA/DC-suburbs market that a Richmond-only seed can't reach — even a 120mi-radius zip locator falls well short of the ~100mi gap between Richmond and the DC suburbs — so `VA_ZIPS` in `fetch-oem-dealer-locators.mjs` gives Northern VA two seeds of its own (Fairfax/Tysons and Alexandria/Arlington) rather than assuming Richmond's radius covers it; confirmed live — Porsche Tysons Corner only shows up once the Northern VA seed is in the mix, distinct from Porsche Richmond. 10 seeds total: Northern VA x2, Richmond, Virginia Beach, Norfolk (Hampton Roads split in two since the metro spans a large bay), Roanoke (southwest), Charlottesville (central), Lynchburg (south-central), Fredericksburg (I-95 corridor), and Winchester (Shenandoah Valley/I-81 corridor). VA came back with 142 in-scope rooftops across 11 brands with a working locator (Toyota 34, Subaru 24, Volkswagen 18, Mazda 16, Mercedes-Benz 12, Acura 8, Kia 8, Mitsubishi 8, Lexus 5, Porsche 5, Mini 4 — Honda/Nissan/Infiniti/Audi/BMW/Volvo stay empty, the same brand-wide dead ends as every prior state), 66 of which came back `NONE`/200-ready on a live `dealer-bot-report.mjs --state=VA` run from a worktree (Cloudflare accounted for the bulk of the rest — 57 of 142 — with a handful of Akamai `HTTP_403`, Vercel-checkpoint, DNS-dead, and one TLS rooftop, no different in kind from every prior state's mix). The raw Toyota locator dump for VA includes an `AutoNation Toyota Leesburg` rooftop (Northern VA) that `acceptVaDealer`'s megadealer filter correctly drops, same as every other state — confirmed with a real fetched-not-invented row in `test/nj_ops.test.js`'s VA block, not just a synthetic one. A live single-brand crawl (Mini, VA's 4 dealers — Checkered Flag MINI, Crown MINI of Richmond, MINI of Alexandria, MINI of Sterling) was run from a worktree to confirm the whole pipeline — locator fetch, policy filter, bot-report, `standalone.js`, VIN enrichment — produces real inventory end to end for VA, not just dealer metadata: Checkered Flag MINI came back Cloudflare-blocked (skipped, matching the bot-report), and the other 3 dealers together surfaced 727 live vehicles (148 from Crown MINI of Richmond, 405 from MINI of Alexandria, 174 from MINI of Sterling), all 727 newly enriched with 0 cache hits and 0 errors; sales-inbox emails were also recovered for 2 of the 3 (MINI of Alexandria, MINI of Sterling). That run's own output files (`data/national_inventory_latest.json` etc.) were deliberately **not** committed here, matching the FL/GA/TX/SC precedent: only the original NJ/NY full-nationwide runs (2026-09-14) are checked into `data/` as the baseline; every state added since has verified its pipeline with a live sample run from a worktree without committing that run's output over the shared baseline, leaving a real full run on the box itself as the source of truth for `data/`.
...
# NJ, NY, FL, GA, TX, SC, then VA, every brand: see "Daily driver" above. Safe to leave in
...
#
# VA (added 2026-09-16, write-va-dealer-files.mjs / src/va_policy.js) is
# the largest state added since TX: 142 in-scope rooftops across every
# brand with a working locator (Toyota 34, Subaru 24, Volkswagen 18,
# Mazda 16, Mercedes-Benz 12, Acura 8, Kia 8, Mitsubishi 8, Lexus 5,
# Porsche 5, Mini 4 — Honda/Nissan/Infiniti/Audi/BMW/Volvo stay empty,
# same brand-wide dead ends as NJ/NY/FL/GA/TX/SC), 66 of which came back
# NONE/200-ready on a live dealer-bot-report run (Cloudflare accounted for
# most of the rest — 57 of 142 — a normal mix, not VA-specific). Real
# timing was only measured for one brand (Mini, 3 of VA's 4 dealers —
# Checkered Flag MINI came back Cloudflare-blocked): 727 vehicles, all
# newly enriched, 0 errors. That is a 3-dealer sample against VA's real
# ready-brand mix of 66 rooftops across 10 brands — do NOT add VA's
# runtime to this box's schedule from that sample, for the same reason
# every prior state's note above insists on a real full run's number.
# VA's 142 rooftops sit between GA's 127 and TX's 312, so it likely adds
# noticeably more than GA did to the combined schedule — but that is a
# scheduling conversation for the user once VA has a real full run's
# number, not something to project from a one-brand sample.
