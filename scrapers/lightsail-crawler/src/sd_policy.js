// South Dakota dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as SD_BRANDS_IN };

export function isSdDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'SD';
}

export function acceptSdDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isSdDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isSdDealer(dealer);
}

export function loadSdDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('SD', { cwd })) {
    ...
    if (!acceptSdDealer(dealer, { brand })) continue;
...
export function writeSdDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'sd');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadSdDealers({ cwd: root, brand: brandName });
