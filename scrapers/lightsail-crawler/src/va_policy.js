// Virginia dealer lists for the Lightsail crawler.
// Same IN/OUT brands and megadealer rules as NJ/NY/FL/GA/TX/SC. Hosts come
...
export { NJ_BRANDS_IN as VA_BRANDS_IN };

export function isVaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'VA';
}

export function acceptVaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isVaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isVaDealer(dealer);
}

export function loadVaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('VA', { cwd })) {
    ...
    if (!acceptVaDealer(dealer, { brand })) continue;
...
export function writeVaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'va');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadVaDealers({ cwd: root, brand: brandName });
