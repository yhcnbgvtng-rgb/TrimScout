// Wyoming dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as WY_BRANDS_IN };

export function isWyDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'WY';
}

export function acceptWyDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isWyDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isWyDealer(dealer);
}

export function loadWyDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('WY', { cwd })) {
    ...
    if (!acceptWyDealer(dealer, { brand })) continue;
...
export function writeWyDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'wy');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadWyDealers({ cwd: root, brand: brandName });
