// Texas dealer lists for the Lightsail crawler.
// Same IN/OUT brands and megadealer rules as NJ/NY/FL/GA. Hosts come only
...
export { NJ_BRANDS_IN as TX_BRANDS_IN };

export function isTxDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'TX';
}

export function acceptTxDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isTxDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isTxDealer(dealer);
}

export function loadTxDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('TX', { cwd })) {
    ...
    if (!acceptTxDealer(dealer, { brand })) continue;
...
export function writeTxDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'tx');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadTxDealers({ cwd: root, brand: brandName });
