// Montana dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as MT_BRANDS_IN };

export function isMtDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'MT';
}

export function acceptMtDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isMtDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isMtDealer(dealer);
}

export function loadMtDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('MT', { cwd })) {
    ...
    if (!acceptMtDealer(dealer, { brand })) continue;
...
export function writeMtDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'mt');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadMtDealers({ cwd: root, brand: brandName });
