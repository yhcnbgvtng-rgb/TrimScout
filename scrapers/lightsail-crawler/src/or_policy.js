// Oregon dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as OR_BRANDS_IN };

export function isOrDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'OR';
}

export function acceptOrDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isOrDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isOrDealer(dealer);
}

export function loadOrDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('OR', { cwd })) {
    ...
    if (!acceptOrDealer(dealer, { brand })) continue;
...
export function writeOrDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'or');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadOrDealers({ cwd: root, brand: brandName });
