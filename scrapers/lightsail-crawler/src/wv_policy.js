// West Virginia dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as WV_BRANDS_IN };

export function isWvDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'WV';
}

export function acceptWvDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isWvDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isWvDealer(dealer);
}

export function loadWvDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('WV', { cwd })) {
    ...
    if (!acceptWvDealer(dealer, { brand })) continue;
...
export function writeWvDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'wv');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadWvDealers({ cwd: root, brand: brandName });
