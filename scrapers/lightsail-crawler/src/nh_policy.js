// New Hampshire dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as NH_BRANDS_IN };

export function isNhDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'NH';
}

export function acceptNhDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isNhDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isNhDealer(dealer);
}

export function loadNhDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('NH', { cwd })) {
    ...
    if (!acceptNhDealer(dealer, { brand })) continue;
...
export function writeNhDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'nh');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadNhDealers({ cwd: root, brand: brandName });
