// Georgia dealer lists for the Lightsail crawler.
// Same IN/OUT brands and megadealer rules as NJ/NY/FL. Hosts come only from
...
export { NJ_BRANDS_IN as GA_BRANDS_IN };

export function isGaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'GA';
}

export function acceptGaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isGaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isGaDealer(dealer);
}

export function loadGaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('GA', { cwd })) {
    ...
    if (!acceptGaDealer(dealer, { brand })) continue;
...
export function writeGaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ga');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadGaDealers({ cwd: root, brand: brandName });
