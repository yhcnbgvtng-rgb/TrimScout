// Colorado dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as CO_BRANDS_IN };

export function isCoDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'CO';
}

export function acceptCoDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isCoDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isCoDealer(dealer);
}

export function loadCoDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('CO', { cwd })) {
    ...
    if (!acceptCoDealer(dealer, { brand })) continue;
...
export function writeCoDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'co');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadCoDealers({ cwd: root, brand: brandName });
