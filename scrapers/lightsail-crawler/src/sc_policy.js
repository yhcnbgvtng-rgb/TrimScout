// South Carolina dealer lists for the Lightsail crawler.
// Same IN/OUT brands and megadealer rules as NJ/NY/FL/GA/TX. Hosts come only
// from official OEM locator dumps and listing-verified VDPs — never
...
export { NJ_BRANDS_IN as SC_BRANDS_IN };

export function isScDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'SC';
}

export function acceptScDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isScDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isScDealer(dealer);
}

export function loadScDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('SC', { cwd })) {
    ...
    if (!acceptScDealer(dealer, { brand })) continue;
...
export function writeScDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'sc');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadScDealers({ cwd: root, brand: brandName });
