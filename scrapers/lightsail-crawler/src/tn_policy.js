// Tennessee dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as TN_BRANDS_IN };

export function isTnDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'TN';
}

export function acceptTnDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isTnDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isTnDealer(dealer);
}

export function loadTnDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('TN', { cwd })) {
    ...
    if (!acceptTnDealer(dealer, { brand })) continue;
...
export function writeTnDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'tn');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadTnDealers({ cwd: root, brand: brandName });
