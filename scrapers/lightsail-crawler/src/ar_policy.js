// Arkansas dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as AR_BRANDS_IN };

export function isArDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'AR';
}

export function acceptArDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isArDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isArDealer(dealer);
}

export function loadArDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('AR', { cwd })) {
    ...
    if (!acceptArDealer(dealer, { brand })) continue;
...
export function writeArDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ar');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadArDealers({ cwd: root, brand: brandName });
