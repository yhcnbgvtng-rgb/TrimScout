// Louisiana dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as LA_BRANDS_IN };

export function isLaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'LA';
}

export function acceptLaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isLaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isLaDealer(dealer);
}

export function loadLaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('LA', { cwd })) {
    ...
    if (!acceptLaDealer(dealer, { brand })) continue;
...
export function writeLaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'la');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadLaDealers({ cwd: root, brand: brandName });
