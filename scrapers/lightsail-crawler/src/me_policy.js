// Maine dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as ME_BRANDS_IN };

export function isMeDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'ME';
}

export function acceptMeDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isMeDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isMeDealer(dealer);
}

export function loadMeDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('ME', { cwd })) {
    ...
    if (!acceptMeDealer(dealer, { brand })) continue;
...
export function writeMeDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'me');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadMeDealers({ cwd: root, brand: brandName });
