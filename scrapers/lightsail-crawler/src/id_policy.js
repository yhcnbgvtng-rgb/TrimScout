// Idaho dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as ID_BRANDS_IN };

export function isIdDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'ID';
}

export function acceptIdDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isIdDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isIdDealer(dealer);
}

export function loadIdDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('ID', { cwd })) {
    ...
    if (!acceptIdDealer(dealer, { brand })) continue;
...
export function writeIdDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'id');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadIdDealers({ cwd: root, brand: brandName });
