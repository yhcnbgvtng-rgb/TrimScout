// Kansas dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as KS_BRANDS_IN };

export function isKsDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'KS';
}

export function acceptKsDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isKsDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isKsDealer(dealer);
}

export function loadKsDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('KS', { cwd })) {
    ...
    if (!acceptKsDealer(dealer, { brand })) continue;
...
export function writeKsDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ks');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadKsDealers({ cwd: root, brand: brandName });
