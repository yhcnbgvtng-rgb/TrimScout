// Alabama dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as AL_BRANDS_IN };

export function isAlDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'AL';
}

export function acceptAlDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isAlDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isAlDealer(dealer);
}

export function loadAlDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('AL', { cwd })) {
    ...
    if (!acceptAlDealer(dealer, { brand })) continue;
...
export function writeAlDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'al');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadAlDealers({ cwd: root, brand: brandName });
