// Nebraska dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as NE_BRANDS_IN };

export function isNeDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'NE';
}

export function acceptNeDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isNeDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isNeDealer(dealer);
}

export function loadNeDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('NE', { cwd })) {
    ...
    if (!acceptNeDealer(dealer, { brand })) continue;
...
export function writeNeDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ne');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadNeDealers({ cwd: root, brand: brandName });
