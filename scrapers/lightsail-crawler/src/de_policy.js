// Delaware dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as DE_BRANDS_IN };

export function isDeDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'DE';
}

export function acceptDeDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isDeDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isDeDealer(dealer);
}

export function loadDeDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('DE', { cwd })) {
    ...
    if (!acceptDeDealer(dealer, { brand })) continue;
...
export function writeDeDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'de');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadDeDealers({ cwd: root, brand: brandName });
