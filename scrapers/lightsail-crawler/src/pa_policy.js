// Pennsylvania dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as PA_BRANDS_IN };

export function isPaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'PA';
}

export function acceptPaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isPaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isPaDealer(dealer);
}

export function loadPaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('PA', { cwd })) {
    ...
    if (!acceptPaDealer(dealer, { brand })) continue;
...
export function writePaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'pa');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadPaDealers({ cwd: root, brand: brandName });
