// Iowa dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as IA_BRANDS_IN };

export function isIaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'IA';
}

export function acceptIaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isIaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isIaDealer(dealer);
}

export function loadIaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('IA', { cwd })) {
    ...
    if (!acceptIaDealer(dealer, { brand })) continue;
...
export function writeIaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ia');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadIaDealers({ cwd: root, brand: brandName });
