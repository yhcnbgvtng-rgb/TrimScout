// New Mexico dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as NM_BRANDS_IN };

export function isNmDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'NM';
}

export function acceptNmDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isNmDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isNmDealer(dealer);
}

export function loadNmDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('NM', { cwd })) {
    ...
    if (!acceptNmDealer(dealer, { brand })) continue;
...
export function writeNmDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'nm');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadNmDealers({ cwd: root, brand: brandName });
