// North Carolina dealer lists for the Lightsail crawler.
// Same IN/OUT brands and megadealer rules as NJ/NY/FL/GA/TX/SC/VA. Hosts come
...
export { NJ_BRANDS_IN as NC_BRANDS_IN };

export function isNcDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'NC';
}

export function acceptNcDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isNcDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isNcDealer(dealer);
}

export function loadNcDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('NC', { cwd })) {
    ...
    if (!acceptNcDealer(dealer, { brand })) continue;
...
export function writeNcDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'nc');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadNcDealers({ cwd: root, brand: brandName });
