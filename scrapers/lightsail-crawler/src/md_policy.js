// Maryland dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as MD_BRANDS_IN };

export function isMdDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'MD';
}

export function acceptMdDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isMdDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isMdDealer(dealer);
}

export function loadMdDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('MD', { cwd })) {
    ...
    if (!acceptMdDealer(dealer, { brand })) continue;
...
export function writeMdDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'md');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadMdDealers({ cwd: root, brand: brandName });
