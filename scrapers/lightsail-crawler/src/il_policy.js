// Illinois dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as IL_BRANDS_IN };

export function isIlDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'IL';
}

export function acceptIlDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isIlDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isIlDealer(dealer);
}

export function loadIlDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('IL', { cwd })) {
    ...
    if (!acceptIlDealer(dealer, { brand })) continue;
...
export function writeIlDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'il');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadIlDealers({ cwd: root, brand: brandName });
