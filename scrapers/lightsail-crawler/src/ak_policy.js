// Alaska dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as AK_BRANDS_IN };

export function isAkDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'AK';
}

export function acceptAkDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isAkDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isAkDealer(dealer);
}

export function loadAkDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('AK', { cwd })) {
    ...
    if (!acceptAkDealer(dealer, { brand })) continue;
...
export function writeAkDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ak');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadAkDealers({ cwd: root, brand: brandName });
