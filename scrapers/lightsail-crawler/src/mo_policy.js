// Missouri dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as MO_BRANDS_IN };

export function isMoDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'MO';
}

export function acceptMoDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isMoDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isMoDealer(dealer);
}

export function loadMoDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('MO', { cwd })) {
    ...
    if (!acceptMoDealer(dealer, { brand })) continue;
...
export function writeMoDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'mo');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadMoDealers({ cwd: root, brand: brandName });
