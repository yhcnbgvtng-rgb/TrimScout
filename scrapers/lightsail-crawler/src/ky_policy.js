// Kentucky dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as KY_BRANDS_IN };

export function isKyDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'KY';
}

export function acceptKyDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isKyDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isKyDealer(dealer);
}

export function loadKyDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('KY', { cwd })) {
    ...
    if (!acceptKyDealer(dealer, { brand })) continue;
...
export function writeKyDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ky');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadKyDealers({ cwd: root, brand: brandName });
