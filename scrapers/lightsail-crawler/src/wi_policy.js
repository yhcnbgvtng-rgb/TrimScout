// Wisconsin dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as WI_BRANDS_IN };

export function isWiDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'WI';
}

export function acceptWiDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isWiDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isWiDealer(dealer);
}

export function loadWiDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('WI', { cwd })) {
    ...
    if (!acceptWiDealer(dealer, { brand })) continue;
...
export function writeWiDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'wi');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadWiDealers({ cwd: root, brand: brandName });
