// Nevada dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as NV_BRANDS_IN };

export function isNvDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'NV';
}

export function acceptNvDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isNvDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isNvDealer(dealer);
}

export function loadNvDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('NV', { cwd })) {
    ...
    if (!acceptNvDealer(dealer, { brand })) continue;
...
export function writeNvDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'nv');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadNvDealers({ cwd: root, brand: brandName });
