// Washington dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as WA_BRANDS_IN };

export function isWaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'WA';
}

export function acceptWaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isWaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isWaDealer(dealer);
}

export function loadWaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('WA', { cwd })) {
    ...
    if (!acceptWaDealer(dealer, { brand })) continue;
...
export function writeWaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'wa');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadWaDealers({ cwd: root, brand: brandName });
