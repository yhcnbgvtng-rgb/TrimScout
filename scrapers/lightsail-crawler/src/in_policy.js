// Indiana dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as IN_BRANDS_IN };

export function isInDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'IN';
}

export function acceptInDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isInDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isInDealer(dealer);
}

export function loadInDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('IN', { cwd })) {
    ...
    if (!acceptInDealer(dealer, { brand })) continue;
...
export function writeInDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'in');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadInDealers({ cwd: root, brand: brandName });
