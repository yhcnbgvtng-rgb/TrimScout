// Ohio dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as OH_BRANDS_IN };

export function isOhDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'OH';
}

export function acceptOhDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isOhDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isOhDealer(dealer);
}

export function loadOhDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('OH', { cwd })) {
    ...
    if (!acceptOhDealer(dealer, { brand })) continue;
...
export function writeOhDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'oh');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadOhDealers({ cwd: root, brand: brandName });
