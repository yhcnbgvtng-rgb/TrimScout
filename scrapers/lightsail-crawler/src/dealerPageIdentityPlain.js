// Plain-JS port of lib/dealerPageIdentity.ts's extractDealerIdentity.
//
// The TS source can't be imported directly into this package (plain Node
// ESM, no TS build step) but has zero imports of its own, so this is a
// mechanical type-strip, not a re-design — keep the two in sync by hand
// when the TS source changes.
//
// Pulling the dealership's own identity out of a fetched page's HTML.
// Dealer sites almost always name themselves in it — schema.org JSON-LD
// first, then og:site_name, then the <title>. Pure string parsing so it
// can be unit-tested against real page shapes.

export const EMPTY_DEALER_IDENTITY = {
  name: null,
  city: null,
  state: null,
  zip: null,
...
// Cloudflare" is not the name of the dealership. Nothing on such a page
// is about the seller.
...
export function isBlockPage(html, httpStatus) {
...
// Names that a block or error page would produce — refused even if the
// block detection above somehow missed the page.
...
// (seller / offeredBy / provider). A Vehicle's `manufacturer` is an
// Organization too, and accepting it at the top level would report "BMW"
// as the dealer.
...
function findDealerNode(node, depth = 0, viaSeller = false) {
...
function identityFromJsonLd(html) {
  ...
  for (const block of blocks) {
    ...
    try {
      ...
      continue; // Malformed JSON-LD is common; skip rather than fail the probe.
    }
...
const NO_LOCATION = { city: null, state: null };
...
function cityStateFromProse(text) {
...
function identityFromDealerNameElement(html) {
...
// The site logo's alt/title is the dealer's own name, and on many templates
// the alt carries the city and state too: alt="Bachrodt BMW Rockford, IL".
function identityFromLogo(html) {
...
function identityFromMetaDescription(html) {
  const description =
...
export function extractDealerIdentity(html, httpStatus) {
