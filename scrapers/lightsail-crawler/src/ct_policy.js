// Connecticut dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as CT_BRANDS_IN };

export function isCtDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'CT';
}

export function acceptCtDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isCtDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isCtDealer(dealer);
}

export function loadCtDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('CT', { cwd })) {
    ...
    if (!acceptCtDealer(dealer, { brand })) continue;
...
export function writeCtDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ct');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadCtDealers({ cwd: root, brand: brandName });
