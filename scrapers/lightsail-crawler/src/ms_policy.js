// Mississippi dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as MS_BRANDS_IN };

export function isMsDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'MS';
}

export function acceptMsDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isMsDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isMsDealer(dealer);
}

export function loadMsDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('MS', { cwd })) {
    ...
    if (!acceptMsDealer(dealer, { brand })) continue;
...
export function writeMsDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ms');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadMsDealers({ cwd: root, brand: brandName });
