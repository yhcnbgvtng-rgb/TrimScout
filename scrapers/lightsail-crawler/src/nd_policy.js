// North Dakota dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as ND_BRANDS_IN };

export function isNdDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'ND';
}

export function acceptNdDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isNdDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isNdDealer(dealer);
}

export function loadNdDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('ND', { cwd })) {
    ...
    if (!acceptNdDealer(dealer, { brand })) continue;
...
export function writeNdDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'nd');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadNdDealers({ cwd: root, brand: brandName });
