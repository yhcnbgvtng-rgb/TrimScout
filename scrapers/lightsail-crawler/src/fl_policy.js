// Florida dealer lists for the Lightsail crawler.
// Same IN/OUT brands and megadealer rules as NJ/NY. Hosts come only from
...
export { NJ_BRANDS_IN as FL_BRANDS_IN };

export function isFlDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'FL';
}

export function acceptFlDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isFlDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isFlDealer(dealer);
}

export function loadFlDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('FL', { cwd })) {
    ...
    if (!acceptFlDealer(dealer, { brand })) continue;
...
export function writeFlDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'fl');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadFlDealers({ cwd: root, brand: brandName });
