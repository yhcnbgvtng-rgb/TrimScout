// Minnesota dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as MN_BRANDS_IN };

export function isMnDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'MN';
}

export function acceptMnDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isMnDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isMnDealer(dealer);
}

export function loadMnDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('MN', { cwd })) {
    ...
    if (!acceptMnDealer(dealer, { brand })) continue;
...
export function writeMnDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'mn');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadMnDealers({ cwd: root, brand: brandName });
