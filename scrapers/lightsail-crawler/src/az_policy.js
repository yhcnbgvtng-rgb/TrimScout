// Arizona dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as AZ_BRANDS_IN };

export function isAzDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'AZ';
}

export function acceptAzDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isAzDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isAzDealer(dealer);
}

export function loadAzDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('AZ', { cwd })) {
    ...
    if (!acceptAzDealer(dealer, { brand })) continue;
...
export function writeAzDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'az');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadAzDealers({ cwd: root, brand: brandName });
