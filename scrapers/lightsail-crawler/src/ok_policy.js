// Oklahoma dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as OK_BRANDS_IN };

export function isOkDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'OK';
}

export function acceptOkDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isOkDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isOkDealer(dealer);
}

export function loadOkDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('OK', { cwd })) {
    ...
    if (!acceptOkDealer(dealer, { brand })) continue;
...
export function writeOkDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ok');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadOkDealers({ cwd: root, brand: brandName });
