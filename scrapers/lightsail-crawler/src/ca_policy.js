// California dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as CA_BRANDS_IN };

export function isCaDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'CA';
}

export function acceptCaDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isCaDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isCaDealer(dealer);
}

export function loadCaDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('CA', { cwd })) {
    ...
    if (!acceptCaDealer(dealer, { brand })) continue;
...
export function writeCaDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'ca');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadCaDealers({ cwd: root, brand: brandName });
