// Vermont dealer lists for the Lightsail crawler.
...
export { NJ_BRANDS_IN as VT_BRANDS_IN };

export function isVtDealer(dealer) {
  return String(dealer?.state || '').trim().toUpperCase() === 'VT';
}

export function acceptVtDealer(dealer, { brand = null } = {}) {
  if (!dealer || !isVtDealer(dealer)) return false;
  return acceptNjDealer({ ...dealer, state: 'NJ' }, { brand }) && isVtDealer(dealer);
}

export function loadVtDealers({ cwd = process.cwd(), brand = null } = {}) {
  if (brand && isNjBrandOut(brand)) return [];
  const out = [];
  const seenName = new Set();
  const seenHost = new Set();
  for (const raw of locatorRowsForState('VT', { cwd })) {
    ...
    if (!acceptVtDealer(dealer, { brand })) continue;
...
export function writeVtDealerFiles(cwd = process.cwd()) {
  const root = path.resolve(cwd);
  const outDir = path.join(root, 'dealers', 'vt');
  fs.mkdirSync(outDir, { recursive: true });
  const written = [];
  for (const brandName of NJ_BRANDS_IN) {
    const dealers = loadVtDealers({ cwd: root, brand: brandName });
