...
export async function POST(request: Request) {
  ...
  const subjectVin = String(body?.subjectVin || body?.vin || "")
    .trim()
    .toUpperCase();
  if (subjectVin.length !== 17 || !isGenesisVin(subjectVin)) {
    return NextResponse.json(
      { error: "A Genesis subject VIN is required." },
      { status: 400 }
    );
  }
  ...
  try {
    const subject = await getGenesisSticker(subjectVin);
    ...
    console.error("genesis-comparables failed:", message);
    return NextResponse.json(
      {
