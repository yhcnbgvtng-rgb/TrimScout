...
async function lookup(opts: { vin?: string; paste?: string; pasteUrl: string | null; request: Request }) {
  ...
  try {
    // The window-sticker fetch itself is free (official GM CWS PDF).
    ...
    const vehicle = await fillMissingYear(gmStickerToVehicle(sticker, listingUrl, listingPrice, currentDealer ?? listingDealerLookup(resolved, listingUrl)));
