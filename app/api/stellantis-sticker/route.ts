...
async function lookup(opts: { vin?: string; paste?: string; pasteUrl: string | null; request: Request }) {
  ...
  try {
    ...
    const vehicle = await resolveVehicleDealer(
      await fillMissingYear(stellantisStickerToVehicle(sticker, listingUrl, listingPrice, null)),
      resolved,
    ...
    console.log(`[stellantis-sticker] ${vin} sticker unavailable → free import: ${reason}`);
    const free = await buildStickerUnavailableImport({ vin, pasteUrl: opts.pasteUrl, source: resolved, makeLabel: "Stellantis", reason }).catch(() => null);
